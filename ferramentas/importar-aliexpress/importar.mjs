...
// Registro local dos nomes-modelo já reservados pela ferramenta. Ele evita
// consultar o catálogo inteiro a cada anúncio só para descobrir se “Cannes”,
// por exemplo, já foi usado. Na primeira importação a ferramenta faz uma
// única leitura do catálogo e cria este arquivo; depois disso, cada publicação
// bem-sucedida acrescenta o nome localmente.
export const ARQUIVO_NOMES_MODELO = path.join(__dirname, '.nomes-modelo.json');
...
export async function lerNomesModeloRegistrados(){
  try {
    const registro = JSON.parse(await readFile(ARQUIVO_NOMES_MODELO, 'utf-8'));
    return nomesValidos(registro?.nomes);
  } catch {
    return null;
  }
}

async function salvarNomesModeloRegistrados(nomes){
  const registro = JSON.stringify({ versao: 1, nomes: nomesValidos(nomes) }, null, 2);
  const temporario = `${ARQUIVO_NOMES_MODELO}.tmp`;
  await writeFile(temporario, registro, { mode: 0o600 });
  await rename(temporario, ARQUIVO_NOMES_MODELO);
  await chmod(ARQUIVO_NOMES_MODELO, 0o600).catch(() => {});
}

// Só chama buscarNomes quando o registro ainda não existe. Assim a consulta
// ao Supabase acontece uma vez para trazer os nomes antigos, e nunca mais na
// rotina normal de importação.
export async function obterNomesModeloRegistrados(buscarNomes){
  const existentes = await lerNomesModeloRegistrados();
  if (existentes !== null) return existentes;

  const nomesIniciais = nomesValidos(await buscarNomes());
  await salvarNomesModeloRegistrados(nomesIniciais);
  return nomesIniciais;
}

// Chamado APÓS salvar o produto. Se a publicação falhar, o nome não é
// reservado e pode ser usado de novo numa tentativa posterior.
export async function registrarNomeModelo(nome){
  if (!nome || typeof nome !== 'string') return;
  const existentes = await lerNomesModeloRegistrados();
  const nomes = nomesValidos([...(existentes || []), nome]);
  await salvarNomesModeloRegistrados(nomes);
}
...
// A Pavan não vende "um anel 2ct qualquer": cada peça recebe um nome de
// modelo próprio. A lista é deliberadamente finita e curada (lugares
// europeus de sonoridade clássica), para a coleção ter unidade em vez de a
// IA inventar palavras aleatórias a cada importação.
export const NOMES_MODELO_EUROPEUS = Object.freeze([
  'Évora', 'Siena', 'Ravena', 'Verona', 'Amalfi', 'Capri', 'Sorrento',
  'Positano', 'Ravello', 'Como', 'Lucca', 'Arezzo', 'Assis', 'Bellagio',
  'Taormina', 'Portofino', 'Florença', 'Vicenza', 'Matera', 'Tivoli',
  'Sevilha', 'Granada', 'Toledo', 'Córdoba', 'Valência', 'Girona',
  'Cádis', 'Segóvia', 'Coimbra', 'Sintra', 'Óbidos', 'Cascais', 'Bruges',
  'Gante', 'Lucerna', 'Montreux', 'Lausanne', 'Annecy', 'Arles', 'Avignon',
  'Chartres', 'Nice', 'Cannes', 'Mônaco'
]);

function textoParaComparar(valor){
...
// Exportada para podermos testar sem chamar a IA. Os nomes que já existem
// no catálogo são excluídos antes de a IA receber um modelo reservado.
export function escolherNomeModeloEuropeu({ nomesUsados = [], semente = '' } = {}){
  const usados = new Set((Array.isArray(nomesUsados) ? nomesUsados : []).map(textoParaComparar));
  const disponiveis = NOMES_MODELO_EUROPEUS.filter(nome => ![...usados].some(titulo => titulo.includes(textoParaComparar(nome))));
  const lista = disponiveis.length ? disponiveis : NOMES_MODELO_EUROPEUS;
  return lista[hashTexto(semente) % lista.length];
}

function garantirModeloNoNome(nome, modelo){
  const titulo = String(nome || '').trim().replace(/\s+/g, ' ');
  if (!titulo || textoParaComparar(titulo).includes(textoParaComparar(modelo))) return titulo;
  const tipo = titulo.match(/^(anel|aliança|brinco|colar|pulseira|joia)\b\s*/i);
  if (!tipo) return `Joia ${modelo} — ${titulo}`;
  const nomeTipo = tipo[1][0].toUpperCase() + tipo[1].slice(1).toLowerCase();
  return `${nomeTipo} ${modelo} — ${titulo.slice(tipo[0].length).trim()}`;
}

export async function formatarComIA({ nomeOriginal, descricaoOriginal, matrizVariacoes, nomesUsados = [] }, chaveApi){
  const modeloReservado = escolherNomeModeloEuropeu({ nomesUsados, semente: `${nomeOriginal}|${descricaoOriginal}` });
...
[Tipo de peça] + [Nome de modelo] + em + [Pedra/Corte]
- Use EXATAMENTE este nome de modelo reservado, logo depois do tipo da peça: "${modeloReservado}". Nunca substitua, traduza, invente outro lugar ou omita esse nome.
- Tipo de peça sempre primeiro (Anel, Aliança, Brinco, Colar, Pulseira...), pois é a palavra que clientes e buscadores usam para encontrar o produto.
- O nome precisa continuar descritivo: não entregue somente "Anel ${modeloReservado}". Exemplo: "Anel ${modeloReservado} em moissanite oval".
- NUNCA coloque CT, quilate, número de quilates ou faixa de quilates no nome. Essa informação aparece no card e nas opções da própria peça.
...
- Curto, natural e elegante: preferencialmente até 12 palavras. Use capitalização normal de português.
...
- Mantém as informações técnicas reais que vieram no texto original (material, tamanho, certificação) — NUNCA inventa informação nova
- Não liste CT, quilates ou faixa de quilates na descrição. As opções corretas já aparecem no seletor e no card da peça.
...
{"nome": "...", "descricao": "<p><strong>...</strong> ...</p>"}`;
  ...
  return {
    nome: garantirModeloNoNome(resultado.nome, modeloReservado),
    descricao: (resultado.descricao || descricaoOriginal || '').trim(),
    modeloReservado
  };
}
...
PADRÃO DO NOME: [Tipo de peça] + [Nome de modelo europeu já presente no nome] + em + [Pedra/Corte]. Preserve exatamente o nome de modelo já presente, salvo se a pessoa pedir explicitamente para trocar o modelo. Mantenha o tipo e os detalhes técnicos relevantes para busca, mas nunca adicione CT ou quilates ao nome.
...
async function modoImportar(url){
  ...
  if (chaveGroq){
    try {
      ...
      const nomesUsados = await obterNomesModeloRegistrados(async () => {
        const { data, error } = await sb.from('produtos').select('nome').limit(500);
  ...
  await registrarNomeModelo(nome).catch(() => {});
